...
gulp.task('build:scss', () => {
  return gulp.src('app/**/*.scss')
    .pipe(sass(opts.sass()))
    .pipe(replace(/url\("\/images/g, 'url("' + CSS_PATH + '/images'))
...
gulp.task("build:updatelibraryLinks", () => {
  const staticHost = "https://storage.googleapis.com/codelab-elements-tmp/";

  return gulp
    .src(["build/codelabs/**/*.html"])
    .pipe(
      replace("https://storage.googleapis.com/codelab-elements/", staticHost)
    )
