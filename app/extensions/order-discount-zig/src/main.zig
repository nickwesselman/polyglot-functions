...
pub fn function(input: api.FunctionInput, allocator: std.mem.Allocator) ?api.FunctionResult {
    ...
    return api.FunctionResult{ .discounts = &[1]api.Discount{api.Discount{
        .value = api.Value{ .percentage = api.Percentage{ .value = config.discountPercentage } },
        .targets = &[1]api.Target{api.Target{ .orderSubtotal = api.OrderSubtotalTarget{ .excludedVariantIds = &[0][]const u8{} } }},
...
pub fn main() u8 {
    ...
    const n = std.io.getStdIn().readAll(&buffer) catch |err| {
        std.debug.print("Error reading function input: {!}", .{err});
        return 1;
    };

    var stream = std.json.TokenStream.init(buffer[0..n]);
